...
function Command.new(options)
	...
	self.Dispatcher = options.Dispatcher  -- The dispatcher that created this command context
	self.Object = options.CommandObject -- The command object (definition)
	self.RawText = options.Text -- The raw text used to trigger this command
	self.Alias = options.Alias -- The command name that was used
	self.Executor = options.Executor -- The player who ran the command
	self.RawArguments = options.Arguments -- Array of strings which are the unparsed values for the arguments
	self.Data = options.Data -- A special container for any additional data the command needs to collect from the client
	self.Guards = options.Guards -- A table of functions where the command will be interrupted if a string is returned	
	
	self.Cmdr = self.Dispatcher.Cmdr -- A quick reference to Cmdr for command context
	self.Name = self.Object.Name -- The command name (not alias)
	self.Group = self.Object.Group -- The group this command is in
	self.Aliases = self.Object.Aliases
	self.Description = self.Object.Description
	self.ArgumentDefinitions = self.Object.Args -- The argument definitions from the command definition

	self.Colors = self.Cmdr.Settings.Colors
	
	self.State = {} -- A table which will hold any custom command state information
	self.Arguments = {} -- A table which will hold ArgumentContexts for each argument
	self.Response = nil -- Will be set at the very end when the command is run and a string is returned from the Run function.
...
function Command:GetRoleSetting(user, Permission)
	return self.Cmdr.Auth.IsAuthorized(user, Permission)
end

function Command:IsAuthorized(user)
	return self.Cmdr.Auth.IsAuthorized(user)
end
